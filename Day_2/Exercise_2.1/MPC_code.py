...
if estimating is False:       
    ...
    sol_optdyn = {'ipopt.max_iter':Sol_itmax, 'ipopt.hessian_constant':Sol_Hess_constdyn,'ipopt.print_level':0,'ipopt.sb':"yes",'print_time':1}#, 'ipopt.tol':1e-10} 
...
timings_solver = timings_solver * 1e3  # scale to milliseconds
print({f'{np.min(timings_solver):.3f}'}, {f'{np.mean(timings_solver):.3f}'}, {f'{np.max(timings_solver):.3f}'})
